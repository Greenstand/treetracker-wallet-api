...
describe('Token', () => {
  ...
  it("getById() with id which doesn't exist, should throw 404", async () => {
    sinon
      .stub(TokenRepository.prototype, 'getById')
  ...
  describe('getTokensByTransferId', () => {
    it('Successfuly', async () => {
      ...
      const fn = sinon
        .stub(TokenRepository.prototype, 'getByTransferId')
        .resolves([{ id: tokenId2 }]);
  ...
  it('completeTransfer', async () => {
    ...
    const _tokens = await tokenService.completeTransfer([token1], transfer);
    expect(updateByIds).calledWith(sinon.match({ wallet_id: transferId1 }), [
      tokenId1,
    ]);
  });
  ...
  it('pendingTransfer', async () => {
    ...
    const _tokens = await tokenService.pendingTransfer([token1], transfer);
    expect(updateByIds).calledWith(
      sinon.match({ transfer_pending: true, transfer_pending_id: transferId1 }),
      [tokenId1],
    );
  });
  ...
  it('cancelTransfer', async () => {
    ...
    const _tokens = await tokenService.cancelTransfer([token1], transfer);
    expect(updateByIds).calledWith(
      sinon.match({ transfer_pending: false, transfer_pending_id: null }),
      [tokenId1],
    );
  });
});
