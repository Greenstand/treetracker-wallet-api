...
(async () => {
  ...
  try {
    ...
    while(remaining) {
      ...
      const rows = await trx('trees').select('*').whereRaw('planter_id IN (select id from planter where organization_id IN ( select entity_id from getEntityRelationshipChildren(?) ) ) AND active = true AND approved = true AND token_id IS NULL limit 3000', [178])
