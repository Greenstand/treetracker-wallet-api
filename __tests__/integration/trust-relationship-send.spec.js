...
describe('Send tokens to \'send\' trust relationship', () => {
    ...
    beforeEach(async () => {
        ...
        relationshipA = await createTrustRelation(walletA, walletB, 'send', 'trusted');
        await createTrustRelation(walletC, walletD, 'receive', 'trusted');
    });
    ...
    it('walletA send tokens to walletE, but the walletE has not accept the "send" trust relationship', async () => {
        ...
        await createTrustRelation(walletA, walletE, 'send', 'requested');
